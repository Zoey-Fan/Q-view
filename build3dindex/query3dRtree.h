...
// This function retrieves the maximum height at a specific 2D location (x, y) within an R-tree.
    ...
    std::function<ELEMTYPE (const DATATYPE&,const ELEMTYPE,const ELEMTYPE)> callback) const
{
  ...
  if(a_node->IsInternalNode())
  {
    ...
    for(int index=0; index < a_node->m_count; ++index){
      ...
      {
		   //~ printf("adadfadsfasdffafdasdfafdasdfd\n");
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
         ...
         !(y>(&a_node->m_branch[index].m_rect)->m_max[1]))
      {
        ...
        if(callback)
        {
          ...
          //std::cout<<"-------h= "<<h<<"---------------\n";
			    if(h>Height)//STOP when meeting the first h
			    {
				   Height=h;
